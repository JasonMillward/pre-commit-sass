...
echo -e "${txtgrn}  $exec_command -i ${args}${txtrst}"
scss_errors_found=false
error_message=""
for path in "${files[@]}"
do
    ${exec_command} -i ${args} ${path} 1> /dev/null
    if [ $? -ne 0 ]; then
        git add ${path}
    fi
done;

if [ "$scss_errors_found" = true ]; then
